...
* [1. Форматы данных: выбор и когда использовать](#1-форматы-данных-выбор-и-когда-использовать)
  * [JSON: самый распространённый формат, чтение/запись с использованием Jackson](#json-самый-распространённый-формат-чтениезапись-с-использованием-jackson)
  * [XML: когда имеет смысл, если API поддерживает старые системы или стандарт](#xml-когда-имеет-смысл-если-api-поддерживает-старые-системы-или-стандарт)
  * [YAML: конфигурация, но не для передачи данных, сравнение с JSON](#yaml-конфигурация-но-не-для-передачи-данных-сравнение-с-json)
  * [Protobuf: эффективность для больших объёмов, бинарный формат, использование с gRPC](#protobuf-эффективность-для-больших-объёмов-бинарный-формат-использование-с-grpc)
  * [CSV, Avro, Parquet: другие специализированные форматы для данных, которые не подходят под JSON/XML](#csv-avro-parquet-другие-специализированные-форматы-для-данных-которые-не-подходят-под-jsonxml)
* [2. Сериализация и десериализация данных в Spring](#2-сериализация-и-десериализация-данных-в-spring)
  * [Jackson как дефолт для JSON: настройка `ObjectMapper`, работа с датами/числами, валидация](#jackson-как-дефолт-для-json-настройка-objectmapper-работа-с-датамичислами-валидация)
  * [`@JsonView`, `@JsonSerialize`, `@JsonDeserialize` для кастомизации сериализации/десериализации](#jsonview-jsonserialize-jsondeserialize-для-кастомизации-сериализациидесериализации)
  * [Kotlin-специфичные аспекты: `data`-классы, `@JsonInclude`, `@JvmField`, проблемы с nullability](#kotlin-специфичные-аспекты-data-классы-jsoninclude-jvmfield-проблемы-с-nullability)
  * [Конфигурация Jackson: обработка исключений, кастомизация путей, использование мапперов для сложных типов](#конфигурация-jackson-обработка-исключений-кастомизация-путей-использование-мапперов-для-сложных-типов)
  * [Десериализация коллекций, вложенных объектов и типов данных в JSON (`Map`, `List<T>`)](#десериализация-коллекций-вложенных-объектов-и-типов-данных-в-json-map-listt)
* [3. Проблемы с сериализацией и производительность](#3-проблемы-с-сериализацией-и-производительность)
  * [Проблемы с большими объектами и глубокой вложенностью: рекурсия, циклические зависимости](#проблемы-с-большими-объектами-и-глубокой-вложенностью-рекурсия-циклические-зависимости)
  * [Настройки Jackson для ограничения глубины, лимитов объектов и избегания StackOverflow](#настройки-jackson-для-ограничения-глубины-лимитов-объектов-и-избегания-stackoverflow)
  * [Влияние сериализации на нагрузку: измерение времени, оптимизация объектов, уменьшение количества полей](#влияние-сериализации-на-нагрузку-измерение-времени-оптимизация-объектов-уменьшение-количества-полей)
  * [Бинарные форматы (например, Protobuf) как альтернатива для сокращения объёмов передачи](#бинарные-форматы-например-protobuf-как-альтернатива-для-сокращения-объёмов-передачи)
* [4. Обработка ошибок в REST API: концепция](#4-обработка-ошибок-в-rest-api-концепция)
  * [Ошибки как ресурсы: код состояния HTTP (4xx/5xx), описание причины и контекста ошибки](#ошибки-как-ресурсы-код-состояния-http-4xx5xx-описание-причины-и-контекста-ошибки)
  * [Формат ошибок: стандартные структуры (RFC 7807, ProblemDetails), семантика `title`, `status`, `detail`, `instance`](#формат-ошибок-стандартные-структуры-rfc-7807-problemdetails-семантика-title-status-detail-instance)
  * [Не использовать 500 для ошибок клиента (например, 404 для несуществующих ресурсов, 400 для неверных данных)](#не-использовать-500-для-ошибок-клиента-например-404-для-несуществующих-ресурсов-400-для-неверных-данных)
  * [Перехват и нормализация ошибок через `@ControllerAdvice`, централизованное маппирование ошибок](#перехват-и-нормализация-ошибок-через-controlleradvice-централизованное-маппирование-ошибок)
* [5. Обработка ошибок в Spring: исключения и их обработка](#5-обработка-ошибок-в-spring-исключения-и-их-обработка)
  * [`@ExceptionHandler`: обработка ошибок на уровне контроллеров, возвращение стандартных ответов](#exceptionhandler-обработка-ошибок-на-уровне-контроллеров-возвращение-стандартных-ответов)
  * [Глобальный обработчик через `@ControllerAdvice`: централизованное управление ошибками для всех контроллеров](#глобальный-обработчик-через-controlleradvice-централизованное-управление-ошибками-для-всех-контроллеров)
  * [Разделение ошибок: специфичные ошибки для бизнес-логики (`UserNotFoundException`), общие ошибки (например, неверный формат запроса)](#разделение-ошибок-специфичные-ошибки-для-бизнес-логики-usernotfoundexception-общие-ошибки-например-неверный-формат-запроса)
  * [Валидация: ошибки валидации через `BindingResult`, подробные сообщения об ошибках](#валидация-ошибки-валидации-через-bindingresult-подробные-сообщения-об-ошибках)
* [6. Стандарты ошибок: ProblemDetails и его реализация](#6-стандарты-ошибок-problemdetails-и-его-реализация)
  * [RFC 7807: структура ошибок, которую можно применять к REST API](#rfc-7807-структура-ошибок-которую-можно-применять-к-rest-api)
  * [Реализация через `ProblemDetail`: создание и кастомизация классов ошибок, расширение стандартных полей для бизнес-логики](#реализация-через-problemdetail-создание-и-кастомизация-классов-ошибок-расширение-стандартных-полей-для-бизнес-логики)
  * [Конвертация ошибок в JSON через `@ResponseStatus` и `@ExceptionHandler`](#конвертация-ошибок-в-json-через-responsestatus-и-exceptionhandler)
* [7. Ошибки валидации: обработка с помощью JSR-303 и Spring](#7-ошибки-валидации-обработка-с-помощью-jsr-303-и-spring)
  * [Использование `@Valid` и `@Validated` для валидации входящих данных](#использование-valid-и-validated-для-валидации-входящих-данных)
  * [Валидация на уровне бина, методов контроллеров, полей и параметров](#валидация-на-уровне-бина-методов-контроллеров-полей-и-параметров)
  * [Интеграция с Jakarta Validation и кастомизация сообщений об ошибках](#интеграция-с-jakarta-validation-и-кастомизация-сообщений-об-ошибках)
  * [Обработка ошибок валидации через `BindingResult` и возвращение подробных сообщений с кодами ошибок](#обработка-ошибок-валидации-через-bindingresult-и-возвращение-подробных-сообщений-с-кодами-ошибок)
* [8. Асинхронные ошибки и эксепшн-хандлинг](#8-асинхронные-ошибки-и-эксепшн-хандлинг)
  * [Обработка ошибок в асинхронных сервисах: `Mono.error`, `Flux.error`, `@Async`-методы](#обработка-ошибок-в-асинхронных-сервисах-monoerror-fluxerror-async-методы)
  * [Реактивные исключения и управление потоком ошибок в WebFlux](#реактивные-исключения-и-управление-потоком-ошибок-в-webflux)
  * [Проблемы с асинхронным выводом: трассировка контекста ошибок, возможность потери данных](#проблемы-с-асинхронным-выводом-трассировка-контекста-ошибок-возможность-потери-данных)
  * [Шаблоны обработки: повторные попытки, отказ по времени, fallback-стратегии](#шаблоны-обработки-повторные-попытки-отказ-по-времени-fallback-стратегии)
* [9. Логирование ошибок и трассировка](#9-логирование-ошибок-и-трассировка)
  * [Логирование ошибок с контекстом: `log.error("Ошибка с данными клиента: {}", customerId, e)`](#логирование-ошибок-с-контекстом-logerrorошибка-с-данными-клиента--customerid-e)
  * [Важность сохранения контекста: traceId, requestId, временные метки для отслеживания цепочки ошибок](#важность-сохранения-контекста-traceid-requestid-временные-метки-для-отслеживания-цепочки-ошибок)
  * [Ошибки и исключения в логах: подробности исключения, стеки вызова](#ошибки-и-исключения-в-логах-подробности-исключения-стеки-вызова)
  * [Трассировка ошибок через MDC (Mapped Diagnostic Context) и прокачка контекста](#трассировка-ошибок-через-mdc-mapped-diagnostic-context-и-прокачка-контекста)
* [10. Тестирование ошибок и исключений](#10-тестирование-ошибок-и-исключений)
  * [Тестирование ошибок через `MockMvc`: проверка правильности возврата ошибок через контроллеры](#тестирование-ошибок-через-mockmvc-проверка-правильности-возврата-ошибок-через-контроллеры)
  * [Интеграционные тесты: проверка проблемных ответов (например, 404, 400, 500)](#интеграционные-тесты-проверка-проблемных-ответов-например-404-400-500)
  * [Тестирование обработки валидации и корректного маппинга ошибок: различные сценарии данных](#тестирование-обработки-валидации-и-корректного-маппинга-ошибок-различные-сценарии-данных)
  * [Набор тестов для проверок стандартов ошибок и их возврата в API (например, используя JUnit 5 и AssertJ)](#набор-тестов-для-проверок-стандартов-ошибок-и-их-возврата-в-api-например-используя-junit-5-и-assertj)
<!-- TOC -->
