...
* [1. Принципы и границы доверия](#1-принципы-и-границы-доверия)
  * [Где валидировать: на периметре (контроллеры) + в домене (инварианты) + в БД (constraint’ы)](#где-валидировать-на-периметре-контроллеры--в-домене-инварианты--в-бд-constraintы)
  * [Trust boundary: всё извне считается недостоверным; валидация до бизнес-логики](#trust-boundary-всё-извне-считается-недостоверным-валидация-до-бизнес-логики)
  * [Слои ответственности: синтаксис/формат → семантика/правила домена → существование ресурсов/прав доступа](#слои-ответственности-синтаксисформат--семантикаправила-домена--существование-ресурсовправ-доступа)
* [2. Таксономия проверок](#2-таксономия-проверок)
  * [Синтаксические (формат email, длины, regex) и семантические (дата «с-до», валюты, статусы)](#синтаксические-формат-email-длины-regex-и-семантические-дата-с-до-валюты-статусы)
  * [Межполевые (кросс-field) и агрегатные (на уровне объекта/коллекции)](#межполевые-кросс-field-и-агрегатные-на-уровне-объектаколлекции)
  * [Онлайн/офлайн: мгновенные проверки и отложенные (например, уникальность по БД)](#онлайнофлайн-мгновенные-проверки-и-отложенные-например-уникальность-по-бд)
  * [Нормализация vs валидация: trim/ASCII-folding/каноникализация до проверки правил](#нормализация-vs-валидация-trimascii-foldingканоникализация-до-проверки-правил)
* [3. Bean Validation в Spring (MVC/Service)](#3-bean-validation-в-spring-mvcservice)
  * [Jakarta Validation 3.x: `@Valid`/`@Validated` в контроллерах и на сервисных методах](#jakarta-validation-3x-validvalidated-в-контроллерах-и-на-сервисных-методах)
  * [`BindingResult` vs исключения: когда возвращать 400/422, когда поднимать исключение](#bindingresult-vs-исключения-когда-возвращать-400422-когда-поднимать-исключение)
  * [Вложенные DTO и коллекции: каскадная валидация, обязательные/опциональные поля](#вложенные-dto-и-коллекции-каскадная-валидация-обязательныеопциональные-поля)
  * [Kotlin-нюансы: nullability как контракт, конструкторная инициализация, default values](#kotlin-нюансы-nullability-как-контракт-конструкторная-инициализация-default-values)
* [4. Аннотации и свои констрейнты](#4-аннотации-и-свои-констрейнты)
  * [Базовые: `@NotNull`/`@NotBlank`/`@Size`/`@Pattern`/`@Email`/`@Positive`/`@PastOrPresent` и т.д.](#базовые-notnullnotblanksizepatternemailpositivepastorpresent-и-тд)
  * [Кастомный constraint: `@interface` + `ConstraintValidator`; повторное использование и тестируемость](#кастомный-constraint-interface--constraintvalidator-повторное-использование-и-тестируемость)
  * [Сообщения и локализация: codes, message bundles, placeholders; понятные тексты для клиента](#сообщения-и-локализация-codes-message-bundles-placeholders-понятные-тексты-для-клиента)
  * [Группы и сценарии: группы по use-case (create/update), fail-fast режим и его влияние](#группы-и-сценарии-группы-по-use-case-createupdate-fail-fast-режим-и-его-влияние)
* [5. Кросс-полевые/агрегатные правила](#5-кросс-полевыеагрегатные-правила)
  * [Class-level валидаторы: согласованность пар («from < to», «type ↔ поля»)](#class-level-валидаторы-согласованность-пар-from--to-type--поля)
  * [Вычисляемые ограничения: сумма элементов, уникальность в коллекции](#вычисляемые-ограничения-сумма-элементов-уникальность-в-коллекции)
  * [Ограничение состояний: конечные автоматы домена и допустимые переходы](#ограничение-состояний-конечные-автоматы-домена-и-допустимые-переходы)
  * [Проверки существования внешних сущностей: справочники/БД/кэши с таймаутами и деградацией](#проверки-существования-внешних-сущностей-справочникибдкэши-с-таймаутами-и-деградацией)
* [6. Входные каналы: что и как проверять](#6-входные-каналы-что-и-как-проверять)
  * [Path/Query/Header/Cookie: типизация, обязательность, множества значений, enum-ы](#pathqueryheadercookie-типизация-обязательность-множества-значений-enum-ы)
  * [Тело запроса: JSON-DTO, multipart (проверка MIME/размера/расширений), потоки](#тело-запроса-json-dto-multipart-проверка-mimeразмерарасширений-потоки)
  * [Ограничения на размер/глубину/сложность JSON, защита от «больших» запросов](#ограничения-на-размерглубинусложность-json-защита-от-больших-запросов)
  * [Идемпотентность и безопасные повторы: Idempotency-Key валидации для мутаций](#идемпотентность-и-безопасные-повторы-idempotency-key-валидации-для-мутаций)
* [7. Реактивность и асинхронность (WebFlux, @Async, messaging)](#7-реактивность-и-асинхронность-webflux-async-messaging)
  * [WebFlux: привязка ошибок (WebExchangeBindException), валидация в функциональных маршрутах](#webflux-привязка-ошибок-webexchangebindexception-валидация-в-функциональных-маршрутах)
  * [Reactor: недопустимость блокировок; валидация как «лёгкий» оператор в цепочке, таймауты/ретраи](#reactor-недопустимость-блокировок-валидация-как-лёгкий-оператор-в-цепочке-таймаутыретраи)
  * [Сообщения (Kafka/JMS): валидация событий до обработки; схемы (JSON Schema/Avro/Proto) на границе](#сообщения-kafkajms-валидация-событий-до-обработки-схемы-json-schemaavroproto-на-границе)
  * [Поведение при сбоях: DLQ/parking lot для «ядовитых» сообщений, метрики отклонений](#поведение-при-сбоях-dlqparking-lot-для-ядовитых-сообщений-метрики-отклонений)
* [8. Ошибки и контракт ответа](#8-ошибки-и-контракт-ответа)
  * [Единый формат ошибок: RFC 7807 (ProblemDetail) с полями field/code для валидации](#единый-формат-ошибок-rfc-7807-problemdetail-с-полями-fieldcode-для-валидации)
  * [Разделять 400 (некорректный формат) и 422 (семантическая ошибка), 409 (конфликт уникальности)](#разделять-400-некорректный-формат-и-422-семантическая-ошибка-409-конфликт-уникальности)
  * [Понятные коды ошибок для клиентов (machine-readable), без утечки PII](#понятные-коды-ошибок-для-клиентов-machine-readable-без-утечки-pii)
  * [Локализация сообщений, корреляция с traceId/requestId, пригодность для фронта и логики ретраев](#локализация-сообщений-корреляция-с-traceidrequestid-пригодность-для-фронта-и-логики-ретраев)
* [9. Производительность, безопасность и эксплуатация](#9-производительность-безопасность-и-эксплуатация)
  * [Fail-fast vs сбор всех ошибок: баланс UX/нагрузки; лимиты на количество нарушений](#fail-fast-vs-сбор-всех-ошибок-баланс-uxнагрузки-лимиты-на-количество-нарушений)
  * [Дедуп и rate-limit на «шумные» ошибки; защита от regex-DoS, чёрные списки символов](#дедуп-и-rate-limit-на-шумные-ошибки-защита-от-regex-dos-чёрные-списки-символов)
  * [БД-constraint’ы как последняя линия: уникальные индексы, NOT NULL, check-ограничения](#бд-constraintы-как-последняя-линия-уникальные-индексы-not-null-check-ограничения)
  * [Частичные обновления (PATCH): валидация «только присланных» полей, мердж с текущим состоянием](#частичные-обновления-patch-валидация-только-присланных-полей-мердж-с-текущим-состоянием)
* [10. Тестирование и инструменты](#10-тестирование-и-инструменты)
  * [Unit-тесты валидаторов и групп; параметризованные негативные кейсы](#unit-тесты-валидаторов-и-групп-параметризованные-негативные-кейсы)
  * [Slice-тесты контроллеров (MockMvc/WebTestClient): 400/422, структура ProblemDetail, локализация](#slice-тесты-контроллеров-mockmvcwebtestclient-400422-структура-problemdetail-локализация)
  * [Контракт-тесты/генеративные проверки (schemathesis/JSON Schema) и синхронизация с OpenAPI](#контракт-тестыгенеративные-проверки-schemathesisjson-schema-и-синхронизация-с-openapi)
  * [Метрики и наблюдаемость: доля отклонённых запросов, топ-правил, алёрты на всплески валидационных ошибок](#метрики-и-наблюдаемость-доля-отклонённых-запросов-топ-правил-алёрты-на-всплески-валидационных-ошибок)
<!-- TOC -->
