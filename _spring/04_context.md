...
* [1. ApplicationContext и Environment: роли и границы](#1-applicationcontext-и-environment-роли-и-границы)
  * [Что такое `ApplicationContext`: контейнер бинов + инфраструктурные сервисы (ресолвер сообщений, событийная шина, конвертеры)](#что-такое-applicationcontext-контейнер-бинов--инфраструктурные-сервисы-ресолвер-сообщений-событийная-шина-конвертеры)
  * [Типы контекстов в Boot 3.x: для servlet, reactive и «голого» приложения; единственный корневой контекст как норма](#типы-контекстов-в-boot-3x-для-servlet-reactive-и-голого-приложения-единственный-корневой-контекст-как-норма)
  * [`Environment` и `PropertySources`: где живут свойства, как происходит резолвинг и приоритеты](#environment-и-propertysources-где-живут-свойства-как-происходит-резолвинг-и-приоритеты)
  * [События старта: когда уже есть `Environment`, но ещё нет всех бинов; где можно вмешаться](#события-старта-когда-уже-есть-environment-но-ещё-нет-всех-бинов-где-можно-вмешаться)
* [2. Источники конфигурации и порядок разрешения](#2-источники-конфигурации-и-порядок-разрешения)
  * [Конфигурация «снаружи»: аргументы командной строки, переменные окружения, системные свойства JVM, файлы `application.yml/.properties`](#конфигурация-снаружи-аргументы-командной-строки-переменные-окружения-системные-свойства-jvm-файлы-applicationymlproperties)
  * [«Config Data API» в Boot: автопоиск `application.yml` в classpath и в `config/`, профилированные файлы `application-<profile>.yml`](#config-data-api-в-boot-автопоиск-applicationyml-в-classpath-и-в-config-профилированные-файлы-application-profileyml)
  * [Расширение локаций: `spring.config.additional-location`, импорт: `spring.config.import` (classpath:, file:, configtree:, configserver: и т.п.)](#расширение-локаций-springconfigadditional-location-импорт-springconfigimport-classpath-file-configtree-configserver-и-тп)
  * [Приоритеты свойств и переопределения: кто «побеждает» при коллизиях; типичные сценарии прод-оверайдов](#приоритеты-свойств-и-переопределения-кто-побеждает-при-коллизиях-типичные-сценарии-прод-оверайдов)
* [3. Профили: активация, группировка, условия](#3-профили-активация-группировка-условия)
  * [Активация профилей: `spring.profiles.active` (env/args), `SPRING_PROFILES_ACTIVE` (env), профили по умолчанию](#активация-профилей-springprofilesactive-envargs-spring_profiles_active-env-профили-по-умолчанию)
  * [Профильные файлы и секции: `application-<profile>.yml`, а также `spring.config.activate.on-profile` внутри одного файла](#профильные-файлы-и-секции-application-profileyml-а-также-springconfigactivateon-profile-внутри-одного-файла)
  * [Группы профилей: логические «сборки» (`spring.profiles.group.*`) для включения нескольких сразу](#группы-профилей-логические-сборки-springprofilesgroup-для-включения-нескольких-сразу)
  * [Условия на бины: `@Profile`, `@ConditionalOnProperty`/`OnClass`/`OnMissingBean` — когда что выбирать](#условия-на-бины-profile-conditionalonpropertyonclassonmissingbean--когда-что-выбирать)
    * [Мини-зависимости для примеров](#мини-зависимости-для-примеров)
* [4. Конфигурационные свойства: типобезопасное биндинг](#4-конфигурационные-свойства-типобезопасное-биндинг)
  * [Зачем `@ConfigurationProperties`: группировка настроек, автодокументация, валидация, удобство тестирования](#зачем-configurationproperties-группировка-настроек-автодокументация-валидация-удобство-тестирования)
  * [Биндинг: «расслабленные» имена, вложенные объекты, коллекции и карты](#биндинг-расслабленные-имена-вложенные-объекты-коллекции-и-карты)
  * [Иммутабельность: конструкторный биндинг по умолчанию в Boot 3.x; почему лучше, чем сеттеры](#иммутабельность-конструкторный-биндинг-по-умолчанию-в-boot-3x-почему-лучше-чем-сеттеры)
  * [Валидация: аннотации Jakarta Validation, отказ от старта при некорректных значениях](#валидация-аннотации-jakarta-validation-отказ-от-старта-при-некорректных-значениях)
* [5. `@ConfigurationProperties` vs `@Value`](#5-configurationproperties-vs-value)
  * [Когда использовать `@ConfigurationProperties`: набор связанных параметров, повторное использование, валидация](#когда-использовать-configurationproperties-набор-связанных-параметров-повторное-использование-валидация)
  * [Когда достаточно `@Value`: единичные флаги/константы; почему плохо «размазывать» `@Value` по сервисам](#когда-достаточно-value-единичные-флагиконстанты-почему-плохо-размазывать-value-по-сервисам)
  * [Плейсхолдеры `${...}` и дефолты (`${X:Y}`), ограничения SpEL; избегаем логики в аннотациях](#плейсхолдеры--и-дефолты-xy-ограничения-spel-избегаем-логики-в-аннотациях)
  * [Инъекция свойств только в конфигурационные/инфраструктурные бины, а не в доменную логику](#инъекция-свойств-только-в-конфигурационныеинфраструктурные-бины-а-не-в-доменную-логику)
* [6. Типы и форматы свойств](#6-типы-и-форматы-свойств)
  * [Встроенные конвертеры: `Duration` (`10s`, `2m`), `DataSize` (`256MB`), `InetAddress`, `URI`, `Enum`](#встроенные-конвертеры-duration-10s-2m-datasize-256mb-inetaddress-uri-enum)
  * [Карты/списки: как задавать в YAML, ключи с точками, «мердж» значений между профилями](#картысписки-как-задавать-в-yaml-ключи-с-точками-мердж-значений-между-профилями)
  * [Секреты и многократное использование: аккуратное составление строк (например, URL из хоста+порта) через плейсхолдеры](#секреты-и-многократное-использование-аккуратное-составление-строк-например-url-из-хостапорта-через-плейсхолдеры)
  * [Локализация и многоязычность: конфиги сообщений отдельно от технических свойств](#локализация-и-многоязычность-конфиги-сообщений-отдельно-от-технических-свойств)
* [7. Метаданные конфигурации и удобство разработки](#7-метаданные-конфигурации-и-удобство-разработки)
  * [Генерация метаданных (`spring-boot-configuration-processor`) для подсказок IDE и документации](#генерация-метаданных-spring-boot-configuration-processor-для-подсказок-ide-и-документации)
  * [Подключение процессора: `annotationProcessor` (Java) или `kapt`/`ksp` (Kotlin), размещение классов свойств в отдельном модуле «contract»](#подключение-процессора-annotationprocessor-java-или-kaptksp-kotlin-размещение-классов-свойств-в-отдельном-модуле-contract)
  * [Описание `@ConstructorBinding`/`@DefaultValue` и Javadoc — как источник помощи в IDE](#описание-constructorbindingdefaultvalue-и-javadoc--как-источник-помощи-в-ide)
  * [Названия иерархий свойств: единый префикс на сервис/модуль, предсказуемые имена](#названия-иерархий-свойств-единый-префикс-на-сервисмодуль-предсказуемые-имена)
* [8. Секреты и внешние конфиги: Docker/K8s/облако](#8-секреты-и-внешние-конфиги-dockerk8sоблако)
  * [Переменные окружения ↔ имена свойств: правила маппинга (верхний регистр, `_` вместо `.`)](#переменные-окружения--имена-свойств-правила-маппинга-верхний-регистр-_-вместо-)
  * [Docker/K8s secrets/configmaps: монтирование как `configtree:`/файлы, отключение логирования значений](#dockerk8s-secretsconfigmaps-монтирование-как-configtreeфайлы-отключение-логирования-значений)
  * [Vault/облачные секреты: отборочный обзор; почему не хранить секреты в гите](#vaultоблачные-секреты-отборочный-обзор-почему-не-хранить-секреты-в-гите)
  * [Разделение конфигов: общий «baseline» + профильные/региональные/тенантные оверрайды](#разделение-конфигов-общий-baseline--профильныерегиональныетенантные-оверрайды)
* [9. Тесты: свойства и профили в тестовой среде](#9-тесты-свойства-и-профили-в-тестовой-среде)
  * [`@ActiveProfiles` для выбора окружения тестов; `@TestPropertySource` для точечного оверрайда](#activeprofiles-для-выбора-окружения-тестов-testpropertysource-для-точечного-оверрайда)
  * [`DynamicPropertySource` для динамических значений (Testcontainers: URL БД/кафки и т.д.)](#dynamicpropertysource-для-динамических-значений-testcontainers-url-бдкафки-и-тд)
  * [Инициализация данных через профили `test`/`it`: отключение тяжёлых интеграций, включение заглушек](#инициализация-данных-через-профили-testit-отключение-тяжёлых-интеграций-включение-заглушек)
  * [Контроль детерминизма: фиксированные значения времени/рандома в свойствах](#контроль-детерминизма-фиксированные-значения-временирандома-в-свойствах)
* [10. Практики и анти-паттерны](#10-практики-и-анти-паттерны)
  * [Не тащить `Environment` в бизнес-код; предпочитать типобезопасные классы свойств](#не-тащить-environment-в-бизнес-код-предпочитать-типобезопасные-классы-свойств)
