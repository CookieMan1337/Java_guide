...
* [1. Зачем логирование и чем оно отличается от метрик/трейсинга](#1-зачем-логирование-и-чем-оно-отличается-от-метриктрейсинга)
  * [Цели: диагностика инцидентов, аудит действий, подтверждение бизнес-событий, подтверждение SLO](#цели-диагностика-инцидентов-аудит-действий-подтверждение-бизнес-событий-подтверждение-slo)
  * [Логи vs метрики vs трейсинг: логи — детали события; метрики — агрегаты и тренды; трейсинг — путь запроса по сервисам](#логи-vs-метрики-vs-трейсинг-логи--детали-события-метрики--агрегаты-и-тренды-трейсинг--путь-запроса-по-сервисам)
  * [Минимальный набор для любого сервиса: структурные логи + базовые метрики HTTP + health](#минимальный-набор-для-любого-сервиса-структурные-логи--базовые-метрики-http--health)
* [2. Уровни логирования и политика уровней](#2-уровни-логирования-и-политика-уровней)
  * [TRACE/DEBUG/INFO/WARN/ERROR: чёткие критерии применения; в проде DEBUG выключен по умолчанию](#tracedebuginfowarnerror-чёткие-критерии-применения-в-проде-debug-выключен-по-умолчанию)
  * [Политика повышения уровня: ошибка клиента ≠ ERROR, временные сбои — WARN с контекстом](#политика-повышения-уровня-ошибка-клиента--error-временные-сбои--warn-с-контекстом)
  * [Переключение уровней на лету (в том числе через Actuator) и контроль шумности](#переключение-уровней-на-лету-в-том-числе-через-actuator-и-контроль-шумности)
* [3. Структурные логи (JSON) и состав полей](#3-структурные-логи-json-и-состав-полей)
  * [Обязательные поля: timestamp, level, logger, thread, message, service.name, version, env](#обязательные-поля-timestamp-level-logger-thread-message-servicename-version-env)
  * [Корреляция: traceId/spanId, внешние correlationId (если есть), requestId](#корреляция-traceidspanid-внешние-correlationid-если-есть-requestid)
  * [Контекст: ключевые business-атрибуты (id операции, сумма, статус), но без ПДн и секретов](#контекст-ключевые-business-атрибуты-id-операции-сумма-статус-но-без-пдн-и-секретов)
  * [Форматирование исключений: stacktrace в отдельном поле, сокращение «шума» общих фреймов](#форматирование-исключений-stacktrace-в-отдельном-поле-сокращение-шума-общих-фреймов)
* [4. Конфигурация логгера в Spring Boot](#4-конфигурация-логгера-в-spring-boot)
  * [Logback как дефолт: `logback-spring.xml`/`application.yml` для уровней и appenders](#logback-как-дефолт-logback-springxmlapplicationyml-для-уровней-и-appenders)
  * [Console + RollingFile (dev/стенд) и AsyncAppender для неблокирующей записи](#console--rollingfile-devстенд-и-asyncappender-для-неблокирующей-записи)
  * [Паттерны/энкодеры для JSON (логгер, метки, поля MDC)](#паттерныэнкодеры-для-json-логгер-метки-поля-mdc)
  * [Разделение технических логов и access-логов (входящие HTTP-запросы) в разные аппендеры](#разделение-технических-логов-и-access-логов-входящие-http-запросы-в-разные-аппендеры)
* [5. Безопасность логов и маскирование](#5-безопасность-логов-и-маскирование)
  * [Маска секретов: токены, пароли, номера карт; регулярки/фильтры до записи](#маска-секретов-токены-пароли-номера-карт-регуляркифильтры-до-записи)
  * [Запрет логирования тел запросов/ответов по умолчанию; whitelist полей для бизнес-событий](#запрет-логирования-тел-запросовответов-по-умолчанию-whitelist-полей-для-бизнес-событий)
  * [Ограничение размера сообщений и частоты (rate limit) для «болтливых» точек](#ограничение-размера-сообщений-и-частоты-rate-limit-для-болтливых-точек)
  * [Политика хранения: срок, доступы, шифрование на диске/транзите](#политика-хранения-срок-доступы-шифрование-на-дискетранзите)
* [6. Корреляция через MDC и базовая интеграция с трейсингом](#6-корреляция-через-mdc-и-базовая-интеграция-с-трейсингом)
  * [MDC (Mapped Diagnostic Context): прокидываем traceId/correlationId во все логи потока запроса](#mdc-mapped-diagnostic-context-прокидываем-traceidcorrelationid-во-все-логи-потока-запроса)
  * [Пропагация заголовков межсервисно (например, `traceparent`, `x-correlation-id`)](#пропагация-заголовков-межсервисно-например-traceparent-x-correlation-id)
  * [Как минимум: записывать traceId в каждый лог и в ответ клиенту — этого хватает для базовой трассировки](#как-минимум-записывать-traceid-в-каждый-лог-и-в-ответ-клиенту--этого-хватает-для-базовой-трассировки)
* [7. Минимальные метрики для начала (Micrometer + Actuator)](#7-минимальные-метрики-для-начала-micrometer--actuator)
  * [HTTP: количество запросов, коды, латентность (p50/p95)](#http-количество-запросов-коды-латентность-p50p95)
  * [Технические: пул потоков, пул соединений к БД/HTTP-клиенту, очередь задач, ошибки](#технические-пул-потоков-пул-соединений-к-бдhttp-клиенту-очередь-задач-ошибки)
  * [Экспорт: включить `/actuator/metrics` и Prometheus-эндпоинт; предварительно — локальный дашборд](#экспорт-включить-actuatormetrics-и-prometheus-эндпоинт-предварительно--локальный-дашборд)
  * [Перекрёстная проверка: метрика ошибок должна совпадать с частотой ERROR/WARN в логах](#перекрёстная-проверка-метрика-ошибок-должна-совпадать-с-частотой-errorwarn-в-логах)
* [8. Health/Info и технические проверки](#8-healthinfo-и-технические-проверки)
  * [`/actuator/health` (liveness/readiness) и `/actuator/info` (версия, git sha) — включить всегда](#actuatorhealth-livenessreadiness-и-actuatorinfo-версия-git-sha--включить-всегда)
  * [Примитивный self-check: проверка ключевых зависимостей (БД/кеш/очередь) с таймаутами](#примитивный-self-check-проверка-ключевых-зависимостей-бдкешочередь-с-таймаутами)
  * [Стандартные теги: service.name, env, region — единообразно для дашбордов и логов](#стандартные-теги-servicename-env-region--единообразно-для-дашбордов-и-логов)
* [9. Логи в контейнерах и прод-окружении](#9-логи-в-контейнерах-и-прод-окружении)
  * [Писать в stdout/stderr, ротацию делегировать платформе (Docker/лог-агенту)](#писать-в-stdoutstderr-ротацию-делегировать-платформе-dockerлог-агенту)
  * [Передача логов в централизованное хранилище (агенты типа filebeat/fluent-bit) с парсингом JSON](#передача-логов-в-централизованное-хранилище-агенты-типа-filebeatfluent-bit-с-парсингом-json)
  * [Единые теги для поиска: приложение, версия, окружение, компонент](#единые-теги-для-поиска-приложение-версия-окружение-компонент)
  * [Базовые дашборды: RPS/коды/латентность + топ-ошибки и их шаблоны сообщений](#базовые-дашборды-rpsкодылатентность--топ-ошибки-и-их-шаблоны-сообщений)
* [10. Базовый чек-лист качества логирования](#10-базовый-чек-лист-качества-логирования)
  * [Есть структурный формат, traceId в каждом сообщении, маскирование секретов — да/нет](#есть-структурный-формат-traceid-в-каждом-сообщении-маскирование-секретов--данет)
  * [Уровни настроены: DEBUG выключен в проде, ERROR только по реальным сбоям — да/нет](#уровни-настроены-debug-выключен-в-проде-error-только-по-реальным-сбоям--данет)
  * [Access-логи отделены, health/metrics включены, метрики согласованы с логами — да/нет](#access-логи-отделены-healthmetrics-включены-метрики-согласованы-с-логами--данет)
  * [Документация: что логируем, что нельзя логировать, где смотреть, как включить DEBUG на узком участке — да/нет](#документация-что-логируем-что-нельзя-логировать-где-смотреть-как-включить-debug-на-узком-участке--данет)
<!-- TOC -->
