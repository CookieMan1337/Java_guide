...
* [1. Что такое авто-конфигурация и как она включается](#1-что-такое-авто-конфигурация-и-как-она-включается)
  * [Роль `@SpringBootApplication` и `@EnableAutoConfiguration`: сборка приложения из «кубиков» в зависимости от classpath и настроек](#роль-springbootapplication-и-enableautoconfiguration-сборка-приложения-из-кубиков-в-зависимости-от-classpath-и-настроек)
  * [Идея «разумных дефолтов с бэкофом»: автоконфиг создаёт бины только если вы их не определили сами](#идея-разумных-дефолтов-с-бэкофом-автоконфиг-создаёт-бины-только-если-вы-их-не-определили-сами)
  * [Где лежит список авто-конфигураций: файл `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` в артефактах автоконфига](#где-лежит-список-авто-конфигураций-файл-meta-infspringorgspringframeworkbootautoconfigureautoconfigurationimports-в-артефактах-автоконфига)
* [2. Жизненный цикл авто-конфигураций при старте](#2-жизненный-цикл-авто-конфигураций-при-старте)
  * [Фазы: сбор `AutoConfiguration.imports` → применение условий → регистрация бинов → post-processors → готовый контекст](#фазы-сбор-autoconfigurationimports--применение-условий--регистрация-бинов--post-processors--готовый-контекст)
  * [Отчёт условий (Condition Evaluation Report): как понять, почему конкретная авто-конфигурация сработала/не сработала](#отчёт-условий-condition-evaluation-report-как-понять-почему-конкретная-авто-конфигурация-сработалане-сработала)
  * [Диагностика: включение подробных логов условий, просмотр причин «позитивных/негативных» матчей](#диагностика-включение-подробных-логов-условий-просмотр-причин-позитивныхнегативных-матчей)
* [3. Условия авто-конфигурации (Conditional*)](#3-условия-авто-конфигурации-conditional)
  * [Базовые: `@ConditionalOnClass`, `@ConditionalOnMissingBean`, `@ConditionalOnBean`, `@ConditionalOnProperty`](#базовые-conditionalonclass-conditionalonmissingbean-conditionalonbean-conditionalonproperty)
  * [Среда выполнения: `@ConditionalOnWebApplication` (servlet/reactive), `@ConditionalOnCloudPlatform`, профили](#среда-выполнения-conditionalonwebapplication-servletreactive-conditionaloncloudplatform-профили)
  * [Правила «бэкофа»: любые создаваемые бины — под `@ConditionalOnMissingBean`, явные точки выключения — через `@ConditionalOnProperty`](#правила-бэкофа-любые-создаваемые-бины--под-conditionalonmissingbean-явные-точки-выключения--через-conditionalonproperty)
* [4. Порядок применения и зависимость авто-конфигураций](#4-порядок-применения-и-зависимость-авто-конфигураций)
  * [`@AutoConfiguration(before/after = …)` и `@AutoConfigureOrder`: управление очередностью, когда есть перекрытия](#autoconfigurationbeforeafter---и-autoconfigureorder-управление-очередностью-когда-есть-перекрытия)
  * [Дробление на несколько конфиг-классов по зонам ответственности: web-слой, клиенты, data-слой, наблюдаемость](#дробление-на-несколько-конфиг-классов-по-зонам-ответственности-web-слой-клиенты-data-слой-наблюдаемость)
  * [Изоляция от пользовательских бинов: не считайте их «данностью», проверяйте наличие через условия](#изоляция-от-пользовательских-бинов-не-считайте-их-данностью-проверяйте-наличие-через-условия)
* [5. Как написать свою авто-конфигурацию](#5-как-написать-свою-авто-конфигурацию)
  * [Структура модулей: `my-lib-autoconfigure` (аннотации, условия, бины) и `my-lib-spring-boot-starter` (только зависимости)](#структура-модулей-my-lib-autoconfigure-аннотации-условия-бины-и-my-lib-spring-boot-starter-только-зависимости)
  * [Класс с `@AutoConfiguration` + конфиг-бины `@Bean` с `@Conditional*`; регистрация в `AutoConfiguration.imports`](#класс-с-autoconfiguration--конфиг-бины-bean-с-conditional-регистрация-в-autoconfigurationimports)
  * [Валидация и свойства: `@ConfigurationProperties` по конструктору; дефолты и однозначные точки выключения](#валидация-и-свойства-configurationproperties-по-конструктору-дефолты-и-однозначные-точки-выключения)
  * [Runtime hints для native-сборок: ресурсы/рефлексия/прокси через `RuntimeHints`/`@ImportRuntimeHints`](#runtime-hints-для-native-сборок-ресурсырефлексияпрокси-через-runtimehintsimportruntimehints)
* [6. Стартеры: назначение, структура, правила](#6-стартеры-назначение-структура-правила)
  * [Что такое «стартер»: тонкая obвязка зависимостей + `*-autoconfigure` модуль](#что-такое-стартер-тонкая-obвязка-зависимостей---autoconfigure-модуль)
  * [Нейминг: `spring-boot-starter-<name>` для потребителей; автоконфиг — `<name>-autoconfigure`](#нейминг-spring-boot-starter-name-для-потребителей-автоконфиг--name-autoconfigure)
  * [Управление версиями через BOM (`spring-boot-dependencies`); никаких фиксированных версий в стартере](#управление-версиями-через-bom-spring-boot-dependencies-никаких-фиксированных-версий-в-стартере)
  * [Минимизация транзитивных зависимостей и отсутствие кода в стартере](#минимизация-транзитивных-зависимостей-и-отсутствие-кода-в-стартере)
* [7. Кастомизация и отключение авто-конфигураций](#7-кастомизация-и-отключение-авто-конфигураций)
  * [Точечное выключение: `spring.autoconfigure.exclude` или `@EnableAutoConfiguration(exclude = …)`](#точечное-выключение-springautoconfigureexclude-или-enableautoconfigurationexclude--)
  * [Перекрытие бинов: объявите свой `@Bean` совместимого типа — автоконфиг «отступит»](#перекрытие-бинов-объявите-свой-bean-совместимого-типа--автоконфиг-отступит)
  * [Тюнинг через свойства: включение/выключение подсекций автоконфига, явные дефолты](#тюнинг-через-свойства-включениевыключение-подсекций-автоконфига-явные-дефолты)
* [4. Порядок применения и зависимость авто-конфигураций](#4-порядок-применения-и-зависимость-авто-конфигураций-1)
  * [`@AutoConfiguration(before/after = …)` и `@AutoConfigureOrder`: управление очередностью, когда есть перекрытия](#autoconfigurationbeforeafter---и-autoconfigureorder-управление-очередностью-когда-есть-перекрытия-1)
  * [Дробление на несколько конфиг-классов по зонам ответственности: web-слой, клиенты, data-слой, наблюдаемость](#дробление-на-несколько-конфиг-классов-по-зонам-ответственности-web-слой-клиенты-data-слой-наблюдаемость-1)
  * [Изоляция от пользовательских бинов: не считайте их «данностью», проверяйте наличие через условия](#изоляция-от-пользовательских-бинов-не-считайте-их-данностью-проверяйте-наличие-через-условия-1)
* [5. Как написать свою авто-конфигурацию](#5-как-написать-свою-авто-конфигурацию-1)
  * [Структура модулей: `my-lib-autoconfigure` (аннотации, условия, бины) и `my-lib-spring-boot-starter` (только зависимости)](#структура-модулей-my-lib-autoconfigure-аннотации-условия-бины-и-my-lib-spring-boot-starter-только-зависимости-1)
  * [Класс с `@AutoConfiguration` + конфиг-бины `@Bean` с `@Conditional*`; регистрация в `AutoConfiguration.imports`](#класс-с-autoconfiguration--конфиг-бины-bean-с-conditional-регистрация-в-autoconfigurationimports-1)
  * [Валидация и свойства: `@ConfigurationProperties` по конструктору; дефолты и однозначные точки выключения](#валидация-и-свойства-configurationproperties-по-конструктору-дефолты-и-однозначные-точки-выключения-1)
  * [Runtime hints для native-сборок: ресурсы/рефлексия/прокси через `RuntimeHints`/`@ImportRuntimeHints`](#runtime-hints-для-native-сборок-ресурсырефлексияпрокси-через-runtimehintsimportruntimehints-1)
* [6. Стартеры: назначение, структура, правила](#6-стартеры-назначение-структура-правила-1)
  * [Что такое «стартер»: тонкая obвязка зависимостей + `*-autoconfigure` модуль](#что-такое-стартер-тонкая-obвязка-зависимостей---autoconfigure-модуль-1)
  * [Нейминг: `spring-boot-starter-<name>` для потребителей; автоконфиг — `<name>-autoconfigure`](#нейминг-spring-boot-starter-name-для-потребителей-автоконфиг--name-autoconfigure-1)
  * [Управление версиями через BOM (`spring-boot-dependencies`); никаких фиксированных версий в стартере](#управление-версиями-через-bom-spring-boot-dependencies-никаких-фиксированных-версий-в-стартере-1)
  * [Минимизация транзитивных зависимостей и отсутствие кода в стартере](#минимизация-транзитивных-зависимостей-и-отсутствие-кода-в-стартере-1)
* [7. Кастомизация и отключение авто-конфигураций](#7-кастомизация-и-отключение-авто-конфигураций-1)
  * [Точечное выключение: `spring.autoconfigure.exclude` или `@EnableAutoConfiguration(exclude = …)`](#точечное-выключение-springautoconfigureexclude-или-enableautoconfigurationexclude---1)
  * [Перекрытие бинов: объявите свой `@Bean` совместимого типа — автоконфиг «отступит»](#перекрытие-бинов-объявите-свой-bean-совместимого-типа--автоконфиг-отступит-1)
  * [Тюнинг через свойства: включение/выключение подсекций автоконфига, явные дефолты](#тюнинг-через-свойства-включениевыключение-подсекций-автоконфига-явные-дефолты-1)
<!-- TOC -->
